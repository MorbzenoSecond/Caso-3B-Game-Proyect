...
const MESSAGE_SCENE = preload("res://Scr/UI/Components/in_game_message.tscn")
...
func level_stats_scalling_setup(new_local_life, new_local_defense, new_local_level):
	...
	if parent_enemy.main_body_part != self:
		...
		return
	liveBarNode.setup_energy(parent_enemy.true_energy_capacity, parent_enemy.actual_energy_capacity)
...
func calculate_damage(damage_multiplier, brute_damage):
	...
	if parent_enemy.main_body_part != self:
		parent_enemy.main_body_part.put_true_damage(real_damage)
...
func put_true_damage(real_damage):
	local_life -= real_damage
	if local_life <= 0:
		visible = false
		remove_from_group("EnemyBodyPart")
...
#region movement reaction Region
